...
def load_data():
    ...
    f1=file_io.FileIO("gs://aiml_harveen/movie/recommender/orig_data/content_item_train.csv", 'r')
    f2=file_io.FileIO("gs://aiml_harveen/movie/recommender/orig_data/content_user_train.csv", 'r')
    f3=file_io.FileIO("gs://aiml_harveen/movie/recommender/orig_data/content_y_train.csv", 'r')
    f6=file_io.FileIO("gs://aiml_harveen/movie/recommender/orig_data/content_item_vecs.csv", 'r')
    f7=file_io.FileIO("gs://aiml_harveen/movie/recommender/orig_data/content_movie_list.csv", 'r')
    f8=file_io.FileIO("gs://aiml_harveen/movie/recommender/orig_data/content_user_to_genre.pickle", 'r')
    
                             
    item_train = genfromtxt(f1, delimiter=',')
    user_train = genfromtxt(f2, delimiter=',')
    y_train    = genfromtxt(f3, delimiter=',')
    bucket = storage_client.bucket('aiml_harveen')
    blob_item_features = bucket.blob('movie/recommender/orig_data/content_item_train_header.txt')
    blob_user_features = bucket.blob('movie/recommender/orig_data/content_user_train_header.txt')
    blob_user_reader = bucket.blob('movie/recommender/orig_data/content_movie_list.csv')
    blob_user_to_genre = bucket.blob('movie/recommender/orig_data/content_user_to_genre.pickle')
    
    with blob_item_features.open("r", newline='') as f:    #csv reader handles quoted strings better
        item_features = list(csv.reader(f))[0]
    with blob_user_features.open("r", newline='') as f:
        user_features = list(csv.reader(f))[0]
    item_vecs = genfromtxt(f6, delimiter=',')

    movie_dict = defaultdict(dict)
    count = 0
#    with open('./data/movies.csv', newline='') as csvfile:
    with blob_user_reader.open("r", newline='') as csvfile:
    ...
    with blob_user_to_genre.open('rb') as f:
